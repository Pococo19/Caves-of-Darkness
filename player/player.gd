...
enum State { Idle, Run, Jump, Fall, Attack, Slide, Wall }

var current_state

func _on_animation_finished():
	if current_state == State.Attack or current_state == State.Slide:
...
func player_falling(_delta):
	...
	if velocity.y > 0 and current_state != State.Attack and current_state != State.Wall:
		current_state = State.Fall
		anims.flip_h = false if direction > 0 else true

func player_idle(_delta):
	if is_on_floor() and current_state != State.Attack  and current_state != State.Slide:
		current_state = State.Idle
...
func player_run(_delta):
	...
	if is_on_floor() and direction != 0 and current_state != State.Attack  and current_state != State.Slide:
		current_state = State.Run
		anims.flip_h = false if direction > 0 else true
		
	if Input.is_action_just_pressed("slide") and current_state != State.Attack and direction != 0 and is_on_floor():
